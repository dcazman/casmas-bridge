#!/usr/bin/env node
// Patches the loadWeather function in anchor/routes/ui.js
const fs = require('fs');
const path = '/srv/mergerfs/warehouse/casmas-bridge/anchor/routes/ui.js';
let src = fs.readFileSync(path, 'utf8');

const oldBlock = `    // ── Weather panel ──────────────────────────────────────────
...
const newBlock = `    // ── Weather panel (uses better_forecast — current + today's forecast) ──
    async function loadWeather(){
      const body=document.getElementById('wxBody');if(!body)return;
      try{
        const r=await fetch('/weather');const d=await r.json();
...
if (!src.includes(oldBlock)) {
  console.error('ERROR: Could not find old loadWeather block to replace');
  process.exit(1);
}

const patched = src.replace(oldBlock, newBlock);
fs.writeFileSync(path, patched, 'utf8');
console.log('Patched anchor/routes/ui.js successfully');
