...
function extractJSON(raw) {
  ...
  const arrStart = s.indexOf('['); const objStart = s.indexOf('{');
...
function flagStuck(ids, reason) {
  ...
  for (const id of ids) { stmt.run(id); console.warn(`[sync] flagged note ${id} as random — ${reason}`); }
}
...
async function callAI(system, userContent) {
  if (USE_OLLAMA) {
    const resp = await fetch(OLLAMA_URL + '/api/chat', {
      ...
      body: JSON.stringify({ model: 'mistral', format: 'json', stream: false, messages: [{ role: 'system', content: system }, { role: 'user', content: userContent }] })
    });
...
router.post('/', async (req, res) => {
  ...
  if (protectedNotes.length) {
    const stmt = db.prepare("UPDATE notes SET status='processed' WHERE id=?");
    for (const n of protectedNotes) stmt.run(n.id);
  }

  const unprotected = pending.filter(n => !PROTECTED_TYPES.has(n.type));
  const commands = [], regular = [];
  ...
  for (const cmd of commands) {
    commandResults = commandResults.concat(processCommands(cmd.text));
    db.prepare('DELETE FROM notes WHERE id=?').run(cmd.id);
  }

  if (!regular.length) {
    setLastSync();
    return res.json({ ok: true, processed: protectedNotes.length, commands: commandResults.length, engine: USE_OLLAMA ? 'ollama' : 'anthropic' });
  }
...
- 6 or below → set uncertain=true

REMINDERS: If a note contains reminder intent, set remind_at to an ISO 8601 datetime string (America/New_York timezone). If no specific time use null.

Return ONLY a JSON array. Each object must have: source_id, type, formatted, tags, open_loops, uncertain, remind_at (string or null).
No markdown. No explanation. Only the JSON array.`;
  ...
  try {
    ...
    try { parsed = extractJSON(text); }
      ...
      flagStuck(regularIds, 'unparseable response');
      setLastSync();
    ...
    if (!results.length) { flagStuck(regularIds, 'empty results'); setLastSync(); return res.json({ ok: true, processed: 0, flagged: regularIds.length }); }
    ...
    db.transaction(items => {
      ...
      for (const it of items) {
        ...
        if (it.uncertain) { if (!seen.has(sid)) { flagUncertain.run(sid); seen.add(sid); } continue; }
        if (!seen.has(sid)) {
          db.prepare("UPDATE notes SET type=?,status='processed',formatted=?,tags=?,open_loops=? WHERE id=?")
            .run(it.type, encrypt(it.formatted), it.tags ? encrypt(it.tags) : null, encrypt(it.open_loops || ''), sid);
          seen.add(sid);
          if (it.remind_at) { const num = nextRemindNum(); db.prepare('UPDATE notes SET remind_at=?,remind_sent=0,remind_num=? WHERE id=?').run(it.remind_at, num, sid); }
        } else {
          const newId = ins.run(it.type, 'processed', encrypt(it.formatted), encrypt(it.formatted), it.tags ? encrypt(it.tags) : null, encrypt(it.open_loops || '')).lastInsertRowid;
          if (it.remind_at) { const num = nextRemindNum(); db.prepare('UPDATE notes SET remind_at=?,remind_sent=0,remind_num=? WHERE id=?').run(it.remind_at, num, newId); }
        }
      }
    })(results);

    const stillPending = db.prepare("SELECT id FROM notes WHERE id IN (" + regularIds.map(() => '?').join(',') + ") AND status='pending'").all(...regularIds);
    if (stillPending.length) flagStuck(stillPending.map(n => n.id), 'not matched');
    ...
    try { flagStuck(regularIds, e.message); } catch {}
    setLastSync();
